/* ===== 기존 본사 시트 흐름 유지 (A안: 저장 → Thankyou로 이동) ===== */
...
const REDIRECT_AFTER_SAVE = "thankyou"; // fixed
...
function setupImageJump(){
  ...
  imgs.forEach(img=>img.addEventListener('click',()=>go()));
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  formEl.addEventListener('submit', async(e)=>{
    ...
    const phone = (fd.get("contact") || (fd.get("contact1")||"")+(fd.get("contact2")||"")+(fd.get("contact3")||""))
      .replace(/\D/g,"");
    data['연락처'] = `'${phone}`;
