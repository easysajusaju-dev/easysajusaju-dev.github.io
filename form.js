...
function setupHourMinuteSync(person){
  ...
  h.addEventListener('change',()=>{ 
    if(h.value===""){ mm.value=""; mm.disabled=true; } 
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  formEl.addEventListener('submit', async (event)=>{
    ...
    try{
      ...
      let productPrice = 34900;
      const priceMap = {
        '종합사주 미니':5000,'신년운세':19900,'종합사주':34900,'재물사주':14900,'결혼사주':12900,
        '연애사주':9900,'연애패키지':19900,'타이밍패키지':26900,'인생패키지':39900,'재회운':29900,'궁합사주':29900
      };
      if(priceMap[productName]) productPrice = priceMap[productName];
