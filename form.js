// form.js (상호 링크 기능 포함 - 최종 완성 전체 코드)
    ...
    window.handlePriceData = function(result) {
        if (result && result.success) {
            ...
            if (priceListContainer) {
                priceListContainer.innerHTML = ''; // 기존 내용 비우기
                ...
                const createPriceSection = (title, products) => {
                    ...
                    products.forEach(product => {
                        ...
                        let linkButton = '';
                        // 1인용 페이지에서 2인용 상품을 볼 때
                        if (!is2P && isFor2P) {
                            linkButton = `<a href="saju_2p.html" class="link-button">2인 신청</a>`;
                        }
                        // 2인용 페이지에서 1인용 상품을 볼 때
                        if (is2P && isFor1P) {
                            linkButton = `<a href="saju_1p.html" class="link-button">1인 신청</a>`;
                        }
