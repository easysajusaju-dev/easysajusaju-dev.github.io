// form.js (전체 동의 & 2인 폼 성별 선택 기능 포함 - 최종 완성 전체 코드)
...
document.getElementById('saju-form').addEventListener('submit', function(event) {
    ...
    data['상품명'] = formData.get('product'); data['이메일'] = formData.get('email');
    
    // 1인 정보 및 신청자 정보 수집
    data['이름1'] = formData.get('p1_name');
    data['양음력1'] = formData.get('p1_solarlunar');
    const birth1 = getBirthDate('p1');
    if (birth1) { [data['생년1'], data['생월1'], data['생일1']] = birth1.split('-'); }
    ...
    if (form.querySelector('[name="p2_name"]')) {
        ...
        if (birth2) { [data['생년2'], data['생월2'], data['생일2']] = birth2.split('-'); }
