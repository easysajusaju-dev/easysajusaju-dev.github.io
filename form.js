// === form.js (롤백 안정판) ===
// 신청 저장 + thankyou 이동만 수행. 결제 로직 없음.
// 날짜 드롭다운/체크박스/이미지 스크롤 모두 복구.
...
function setupAgreement(){
  ...
  agreeAll.addEventListener('change', ()=>{
    items.forEach(cb => { if(cb) cb.checked = agreeAll.checked; });
    update();
  });
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  formEl.addEventListener('submit', async (event)=>{
    ...
    try{
      ...
      if(b1){ const [yy,mm,dd]=b1.split('-'); data['생년1']=yy; data['생월1']=mm; data['생일1']=dd; }
      ...
      if(formEl.querySelector('[name="p2_name"]')){
        ...
        if(b2){ const [y2,m2,d2]=b2.split('-'); data['생년2']=y2; data['생월2']=m2; data['생일2']=d2; }
      ...
      // 저장 요청(서버가 JSON/텍스트 OK 어느 걸 보내도 통과)
      const body = new URLSearchParams(data);
      const r = await fetch(APPS_SCRIPT_URL, { method:'POST', body });
      const t = await r.text();
      let j; try{ j = JSON.parse(t); }catch(_){ j = { success: true }; }
      if(!j || !j.success) throw new Error('신청 저장 실패');

      // 원래 플로우: 저장 성공 → thankyou 이동
