...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  formEl.addEventListener('submit',async (event)=>{
    ...
    try{
      ...
      const getBirth=(p)=>{
        const y=fd.get(`${p}_birth_year`);
        const m=fd.get(`${p}_birth_month`);
        const d=fd.get(`${p}_birth_day`);
        return (y&&m&&d)?`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`:'';
      };

      let contact=fd.get('contact')||'';
      ...
      data['상품ID']=productId;     // 서버용
      data['상품명']=productName;   // 시트용
      ...
      // ===== 결제 전 단계: 일단 thankyou로 이동 (가격은 서버에서) =====
      window.location.href=`thankyou.html?oid=${orderId}&product=${encodeURIComponent(productName)}`;
